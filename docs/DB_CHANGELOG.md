...
| When (UTC) | Who | What changed | From → To | Script / How |
|------------|-----|--------------|-----------|--------------|
| 2026-02-10 | Backend | Leasing dashboard snapshot table | Added leasing.DashboardSnapshot (Id, Payload NVARCHAR(MAX), BuiltAt, AsOf). GET /api/leasing/dashboard serves from snapshot when present; rebuilt after sync. | schema/create_leasing_schema.sql (add DashboardSnapshot), api leasingRepository, leasingController, leasingDashboardService |
| 2026-02-03 | Backend (guide) | Banking covenant reminder settings | banking.Covenant: added ReminderEmails (NVARCHAR(MAX)), ReminderDaysBefore (NVARCHAR(100), e.g. "7,14,30"). API GET/POST/PUT covenants return/accept as arrays; GET /api/banking/email-templates; POST /api/banking/covenants/:id/send-reminder (auth). | schema/add_covenant_reminder_columns.sql, api bankingController, bankingRoutes |
| 2026-02-03 | Backend (guide) | Land Development Contacts (core.Person + extension) | Replaced standalone pipeline.LandDevelopmentContact with pipeline.LandDevelopmentContactExtension (ContactId FK → core.Person.PersonId). List = Person LEFT JOIN extension; create = insert Person then extension; update = both; delete = extension only. API uses ContactId (PersonId) in URLs and responses. send-reminder uses Person email. Legacy table dropped if present. | schema/add_land_development_contact_extension.sql, api landDevelopmentController, landDevelopmentRoutes |
